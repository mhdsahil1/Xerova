...
export function EditReportDialog({
  ...
  useEffect(() => {
    if (report) {
      ...
      setType(report.type || "investigation");
      setStatus(report.status || "draft");
      setSummary(report.summary || "");
      setRiskScore(report.riskScore ?? 50);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!report?._id || !title.trim()) return;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[94vw] sm:max-w-[94vw] md:max-w-4xl lg:max-w-5xl max-h-[92vh] overflow-y-auto bg-card border-border">
        <DialogHeader>
          ...
          <DialogDescription>
            Modify report parameters, findings, IOC list, and executive summary.
          </DialogDescription>
        </DialogHeader>
