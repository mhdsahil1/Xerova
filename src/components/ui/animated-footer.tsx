...
export interface AnimatedFooterProps {
  ...
  headingLines?: string[];
  /** Words to morph through in place of static heading. */
  morphWords?: string[];
  /** Subtext rendered beneath morphing words. */
  morphSubtext?: string;
  /** Duration (ms) each morph word is displayed. */
  morphInterval?: number;
  /** Custom inner element for display. */
...
export function AnimatedFooter({
  ...
  return (
    ...
    >
      ...
      {/* Display headings or MorphText */}
      <div className="absolute inset-x-0 bottom-0 flex flex-col items-center justify-end pb-6 px-4 z-10">
        {children ? (
          children
        ) : morphWords && morphWords.length > 0 ? (
          <MorphText
            words={morphWords}
            interval={morphInterval}
            subtext={morphSubtext}
            fontSize="clamp(2rem, 10cqw, 6.5rem)"
            className="w-full pb-1"
            textClassName="text-foreground font-black tracking-tight"
            subtextClassName="text-cyan-400/90 font-mono text-xs md:text-sm tracking-widest mt-3"
          />
        ) : (
